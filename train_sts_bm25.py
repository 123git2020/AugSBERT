...
logging.info("Step 1: Train cross-encoder: ({}) with ".format(model_name))
...
#Train the cross-encoder model or load a existing model
...
for t in tokens:
    ...
    if(len(k_sent)==4):
        ...
        silver_data.append((token2sent[" ".join(t)], token2sent[" ".join(k_sent[3][0])]))
...
# Train the bi-encoder model or load an existing model
...
# test on the augmented-sbert model
